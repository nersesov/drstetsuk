    ...
    var NicolasInit = {
        ...
        accordion: function() {
            ...
            $('.accordion_header').on('click', function() {
                var li = $(this).closest('.accordion');
                if (li.hasClass('active')) {
                    li.removeClass('active').find('.accordion_content').slideUp();
                } else {
                    $('.accordion.active').removeClass('active').find('.accordion_content').slideUp();
...
function getRandomInt(min, max) {
  ...
  window.addEventListener('scroll', function() {
	...
	var cloudTop = cloudContainer.offsetTop;
	var windowHeight = window.innerHeight;
	var scrollPosition = window.scrollY;
  
	if (scrollPosition > cloudTop - windowHeight / 2) {
	  var diseases = document.querySelectorAll('.disease');
	  diseases.forEach(function(disease) {
		disease.style.opacity = '1';
		var randomX = getRandomInt(-150, 150); // Случайная позиция по оси X
		var randomY = getRandomInt(-150, 150); // Случайная позиция по оси Y
		disease.style.transform = 'translate(' + randomX + 'px, ' + randomY + 'px)';
	  });
  
	  var healthWord = document.querySelector('.health-word');
	  healthWord.style.opacity = '1';
	} else {
	  var diseases = document.querySelectorAll('.disease');
	  diseases.forEach(function(disease) {
		disease.style.opacity = '0';
		disease.style.transform = 'translate(-50%, -50%)'; // Вернуть в центр
	  });
  
	  var healthWord = document.querySelector('.health-word');
	  healthWord.style.opacity = '0';
	}
  });
